...
```bash
docker pull ghga/encryption-key-store-service:5.1.2
```

Or you can build the container yourself from the [`./Dockerfile`](./Dockerfile):
```bash
# Execute in the repo's root dir:
docker build -t ghga/encryption-key-store-service:5.1.2 .
```
...
docker run -p 8080:8080 ghga/encryption-key-store-service:5.1.2 --help
```
