...
#
"""A mock of the Lox24 SMS gateway, based on the service commons `MockedApi`."""
...
class Lox24Mock(MockedApi):
    ...
    def __init__(self, *, base_url: str, auth_token: str, auth_token_header: str):
        """Serve the Lox24 API at `base_url`, accepting `auth_token`."""
    ...
    @endpoint("POST", SEND_SMS_PATH)
    def send_sms(self, request: Request) -> Response:
        """Respond with the configured status code, rejecting an unexpected request."""
