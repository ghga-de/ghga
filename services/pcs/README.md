...
```bash
docker pull ghga/purge-controller-service:5.0.4
```

Or you can build the container yourself from the [`./Dockerfile`](./Dockerfile):
```bash
# Execute in the repo's root dir:
docker build -t ghga/purge-controller-service:5.0.4 .
```
...
docker run -p 8080:8080 ghga/purge-controller-service:5.0.4 --help
```
