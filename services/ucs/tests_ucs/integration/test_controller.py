...
async def test_failed_interrogation_file_overwrite_protection(
    ...
    # Try to initiate a new file upload for this same alias, expect error
    expected_error = (
        nullcontext()
        if overwrite
        else pytest.raises(UploadControllerPort.FileUploadAlreadyExists)
    )
    async with set_correlation_id(uuid4()):
        with expected_error:
            file_id, _ = await controller.initiate_file_upload(
...
async def test_failed_interrogation_object_survives_cleanup(
    ...
    # Verify that the FileUpload was updated to 'failed-interrogation'
    failed_file = file_upload_collection.find_one(
        {"_id": file_id, "__metadata__.deleted": False}
    )
    assert failed_file is not None
    assert failed_file["state"] == "failed-interrogation"
    assert await s3_storage.does_object_exist(
        bucket_id=inbox_bucket_id, object_id=object_id
    )
