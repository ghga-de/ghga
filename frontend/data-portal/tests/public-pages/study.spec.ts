...
test('can show the study details page', async ({ page }) => {
  ...
  const h1 = page.getByRole('heading', { level: 1, name: 'Study Details' });
  ...
  const h2 = page.getByRole('heading', { level: 2, name: 'Test Study' });
  await expect(h2).toBeVisible();

  const h3 = page.getByRole('heading', { level: 3, name: 'Datasets' });
