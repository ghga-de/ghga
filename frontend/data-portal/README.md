...
Since the move into the monorepo the hooks come from the root `.pre-commit-config.yaml` rather than from Husky ([ADR-0036](../../docs/adrs/0036-pre-commit-hooks.md)); ESLint and Prettier run over the files you touched, out of this package's own `node_modules`, so they are the same versions `pnpm lint` and `pnpm format:check` use. Install them once per clone with `just hooks` from the repo root — the dev container already does.
