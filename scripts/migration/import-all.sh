...
#   LEGACY_DIR=$PWD/.legacy_repos scripts/migration/import-all.sh        # use local mirrors
#   scripts/migration/import-all.sh                                       # clone from ghga-de
#
...
import_row() {
  ...
  log "importing ${source} (${subpath}) -> ${dest}"
  ...
  git -C "$MONOREPO" merge --allow-unrelated-histories --no-edit \
      -m "Import ${source} (${subpath}) into ${dest} (history-preserving)" \
      "${remote}/${BRANCH}"
