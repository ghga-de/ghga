...
def repo(tmp_path):
    ...
    (adrs / "0000-template.md").write_text("# ADR-NNNN — {Title}\n" + BODY)
    (adrs / "0001-first.md").write_text(_adr("0001", "First"))
    (adrs / "0002-second.md").write_text(_adr("0002", "Second"))
...
def test_valid_set_regenerates_index_once(repo, capsys):
    ...
    assert "| [0001](adrs/0001-first.md) | First | accepted | docs |" in readme
...
def test_frontmatter_rules(repo, fields, expected):
    (repo / "docs/adrs/0001-first.md").write_text(_adr("0001", "First", fields))
...
def test_file_name_title_and_duplicates(repo):
    ...
    (adrs / "0002-again.md").write_text(_adr("0002"))
    (adrs / "0004-wrong-number.md").write_text(_adr("0005"))
    _, problems = adr_check.load_adrs(repo)
    assert any("0003_bad.md: file name" in p for p in problems)
    assert any("number 0002 is taken" in p for p in problems)
    assert any("0004-wrong-number.md: first line" in p for p in problems)
...
def test_headings_out_of_order_and_extra_level_two(repo):
    ...
    (repo / "docs/adrs/0001-first.md").write_text(_adr("0001", body=swapped))
    extra = BODY + "\n### Further\n\n## Appendix\n"
    (repo / "docs/adrs/0002-second.md").write_text(_adr("0002", body=extra))
    _, problems = adr_check.load_adrs(repo)
    assert any(p.startswith("0001-first.md: headings") for p in problems)
...
def test_supersession_must_be_symmetric(repo):
    ...
    (repo / "docs/adrs/0001-first.md").write_text(
        _adr("0001", fields=superseded + "superseded-by: [ADR-0002]\n")
    )
    _, problems = adr_check.load_adrs(repo)
    assert problems == [
        "0001-first.md: superseded by ADR-0002, which lacks supersedes: [ADR-0001]"
    ]
    (repo / "docs/adrs/0002-second.md").write_text(
        _adr(
            "0002",
...
def test_superseded_status_needs_superseded_by(repo):
    ...
    assert adr_check.load_adrs(repo)[1] == [
        "0001-first.md: status superseded needs superseded-by"
    ]
...
def test_references(repo, capsys):
    (repo / "notes.md").write_text(
        ...
        "[ok](docs/adrs/0002-second.md) [gone](../adrs/0002-renamed.md)\n"
    )
    (repo / "docs/adrs/0002-second.md").write_text(
        _adr("0002", body=BODY + "\nSee [first](0001-first.md), [x](0001-old.md).\n")
    )
    code, out = _run(
        repo, capsys, "--refs", "notes.md", "docs/adrs/0002-second.md", "gone.md"
    )
    assert code == 1
    ...
    assert lines[:4] == [
        ...
        "notes.md:3: link to docs/adrs/0002-renamed.md, which does not exist",
    ]
    assert len(lines) == 5
    assert lines[4].startswith("docs/adrs/0002-second.md:")
    assert lines[4].endswith("link to docs/adrs/0001-old.md, which does not exist")
