...
- `release.yaml` drafts one for a `ghga/` tag. It also tags the companions, `metldata`
  and `ghga-datasteward-kit`, as `name/X.Y.Z` with the platform version and drafts
  theirs, unless nothing they ship has changed.
