...
- **Integration testing on Kubernetes** (kind in CI, minikube/colima locally) using the same
...
- The ex-`archive-test-bed` BDD + Playwright suite moves to `testbed/` and runs **the same
  ...
  cluster (CI) / **minikube** (local). "What you install == what CI tests."
...
- Hosted under **`github.com/lkuchenb`**; images → **DockerHub `lkuchenb`**; charts → GHCR
  under `lkuchenb`. **No PyPI publishing during the sandbox** (internal deps are source, so it
  isn't needed; PyPI's global namespace also blocks reusing prod names). PyPI pipelines are
  authored but disabled until cutover.
