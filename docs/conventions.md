...
suffix; body is the PR description cut to three to five bullets, wrapped at 72. Mark a breaking change `feat(upload)!:` with a `BREAKING CHANGE:` footer. Do not repeat
the YouTrack key — `(#<PR>)` leads to it. **No `Co-authored-by:` line for an agent in a
commit**, whatever the tool says; human co-authors keep their trailers. Where an agent
substantially drafted the change — not merely suggested, explained or proofread it — the PR
description closes with "Generated by Claude Code" or "Co-authored by Claude Code", naming the
model only where that matters.
