...
- Popularity:
  - How popular is the framework? How widely used is it? How much support is available on the Internet? How easy is it to attract developers for this framework?
  - Here, React is clearly the most popular one and highly visible on the Internet, but Angular is also used by many companies.
- Maintenance:
...
- Testing support:
  ...
  - Angular comes with support for unit and end-to-end tests, and the CLI tool also provides scaffolding for tests. Vue and React also provide test utilities, but the Angular test environment may be a bit more comprehensive.
- Security:
...
- Internationalization:
  - Since we only plan to support the English language, we can ignore this criterion as well.
