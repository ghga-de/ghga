...
- Add a check to the start of `interrogate_file()` that consults both `max_concurrent_files` and `max_interrogation_memory_mib`, so a file only starts once there is room under both limits. `asyncio.Semaphore` alone can gate based on `max_concurrent_files`, but it can't handle the memory limitation side of things. That check should be based on part size.
