...
The existing functionality for the upload is outdated and needs to be replaced with logic for the new concept described in [Lynx Boreal](./epic-0076-lynx-boreal/README.md).
...
- Does some precondition checks for functionality further down in the call hierarchy, but those functions do additional precondition checks
...
- Needs httpx.AsyncClient and WorkPackageAccessor
- Calls functionality from `downloading/api_calls.py`, passing either the client or WorkPackageAccessor along
...
Centralizing all API calling functionality that is currently spread across different parts of the codebase should allow for easier extensibility in the future.
