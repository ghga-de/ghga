...
Additionally, a periodic job should check if files remain in staging that are no longer needed and (for now) log affected objects. As objects should be purged asap, objects remaining in the bucket mean there is probably a logic bug somewhere.
