...
Moving the existing tenacity based retry logic should be straightforward, converting the AsyncRetrying to a property of the transport, wrapping the `self._transport.handle_async_request(request)` call inside its own `handle_async_request` method.
...
For now, the `Retry-After` response is saved as shared state and applied to a configurable number of subsequent requests, falling back to an exponential backoff strategy if no `Retry-After` header is present in the HTTP 429 response.
