...
We should also update UCS and RS to allow 'failed' files to be deleted from boxes even when they're locked. Otherwise users/Data Stewards have to do an inconvenient dance where they unlock-delete-lock, and that's not fun. To do this in the UCS we need to reposition the call to the `_get_unlocked_box()` method in `remove_file_upload()` so it comes _after_ fetching the FileUpload. Furthermore, `_get_unlocked_box()` should be renamed `_get_box()` with a parameter `unlocked_only: bool`. Then `remove_file_upload()` will specify `unlocked_only` based on the FileUpload state and `decrypted_sha256` field. This `_get_box()` method can also replace the other spots where we call FileUploadBox DAO's `get_by_id()`.
