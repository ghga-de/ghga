...
- **event_payload**: the actual data that is shipped with the event. It has to be JSON-serializable.
...
with the providers after running a protocol-compliant test operation. Moreover, if the providers are sufficiently tested for compliance with the protocol, the value of testing translators with production-ready providers seems limited. It seems more practical to only
...
is also not applicable to local unit and integration tests. It is probably more practical to approach the setup and teardown of test states
