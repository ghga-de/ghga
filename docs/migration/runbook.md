...
- [ ] Archive the old repos (keep read-only for history/provenance); update external docs that
      ...
      `ghga-jsonsubschema`, `archive-test-bed` and `ghga-service-commons` are archived
      and their rows removed from `repos.tsv` — see the "Retired" block there for the
      last commit merged from each. Verify a repo is fully synced (§6) before dropping
      its row.
