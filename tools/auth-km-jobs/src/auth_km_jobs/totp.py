...
def re_encrypt_tokens() -> None:
    ...
    client: MongoClient = MongoClient(
        config.mongo_dsn, serverSelectionTimeoutMS=MONGO_DB_TIMEOUT
    )
    ...
    previous_validator = _install_collection_write_block(
        db, config.user_tokens_collection
    )
    needs_restore = False
...
def _re_encrypt_all(
    ...
    old_box: nacl.secret.SecretBox,
    new_box: nacl.secret.SecretBox,
) -> None:
...
def _remove_collection_write_block(
    db: Database, name: str, previous: dict[str, Any]
) -> None:
