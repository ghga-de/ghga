...
def _render_chart_case(composed_values: dict[str, Any],) -> str:
    output = subprocess.run(
        ["helm", "template", "--namespace", "test", "--values", "-", str(DUMMY_CHART_DIR)],
        capture_output=True,
        text=True,
...
def _load_objects_from_yaml(yaml_str: str) -> dict[str, Any]:
    ...
    for obj in objects:
        if obj.get("kind") in objects_dict:
            raise ValueError(f"Duplicate object kind '{obj.get('kind')}' found in rendered manifests. All objects must have unique 'kind' fields.")
        else:
...
def compose_values(*filenames: str) -> dict[str, Any]:
    ...
    for name in filenames:
        composed = _deep_merge(composed, _load_yaml(CASES_DIR / name))
    return composed

@pytest.fixture
def rendered_chart(_dependency_update):
    def _factory(*filenames: str):
        composed_values = compose_values(*filenames)
        return _load_objects_from_yaml(_render_chart_case(composed_values))
    return _factory
