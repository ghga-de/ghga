...
```
helm install ars oci://registry-1.docker.io/ghga/ars-chart
```

## Source

Part of the [GHGA monorepo](https://github.com/ghga-de/ghga/tree/main/services/access-request-service). See
[values.yaml](https://github.com/ghga-de/ghga/blob/main/services/access-request-service/values.yaml) for the
...
| `executable` | Executable name and arguments (will be combined into a shell command) | `"ars"` |
...
| `image.repository` | Image repository path (create_charts.py fills this in per member) | `"ghga/ars"` |
...
| `mongodb.dbName` | Database name; combined with dbNamePrefix and injected into config.yaml as db_name. NOTE: mongodb.dbName is the fallback used when the top-level dbName (set per-member, not defaulted here) is empty | `"access-request"` |
...
| `apiBasePath` | Public API path prefix; combined with apiBasePathPrefix (set by an aliasing umbrella) and injected into config.yaml as api_root_path | `"/ars/"` |
| `serviceName` | Logical service name; combined with serviceNamePrefix and injected into config.yaml as service_name | `"ars-consumer"` |
...
| `config.download_access_url` | URL pointing to the internal download access API. | `null` |
...
| `config.access_upfront_max_days` | The maximum lead time in days to request access grants | `180` |
| `config.access_grant_min_days` | The minimum number of days that the access will be granted | `7` |
| `config.access_grant_max_days` | The maximum number of days that the access can be granted | `730` |
| `config.access_grant_max_extend` | This is a factor that the maximum number of days is multiplied with for data stewards. Set this to 1 to disable extension. | `5` |
...
| `config.service_name` | Short name of this service. NOTE: this chart's configmap.tpl always overwrites config.service_name with the value computed from `serviceName` - a value set directly under config.service_name is silently discarded. Set `serviceName` instead. | `"ars"` |
...
| `configPrefix` | Prefix for the generated CONFIG_YAML env var and every Vault Agent-injected env var; create_charts.py derives this automatically from the package name | `"ars"` |
...
> Docker Hub caps this overview at 25000 characters, so the table above stops partway through this chart's parameters. The full list, with defaults and descriptions, is in [values.schema.json](https://github.com/ghga-de/ghga/blob/main/services/access-request-service/values.schema.json).
