...
[values.yaml](https://github.com/ghga-de/ghga/blob/main/deploy/charts/metldata/values.yaml)
...
| `image.repository` | Image repository path (create_charts.py fills this in per member) | `"ghga/metldata"` |
...
| `mongodb.dbName` | Database name; combined with dbNamePrefix and injected into config.yaml as db_name. NOTE: mongodb.dbName is the fallback used when the top-level dbName (set per-member, not defaulted here) is empty | `"metadata-artifacts"` |
...
| `apiBasePath` | Public API path prefix; combined with apiBasePathPrefix (set by an aliasing umbrella) and injected into config.yaml as api_root_path | `"/metldata/"` |
| `serviceName` | Logical service name; combined with serviceNamePrefix and injected into config.yaml as service_name | `"metldata"` |
...
| `config.artifact_infos` | Information for artifacts to be queryable via the Artifacts REST API. | `null` |
...
| `config.loader_token_hashes` | Hashes of tokens used to authenticate for loading artifact. | `null` |
| `config.primary_artifact_name` | Name of the artifact from which the information for outgoing change events is derived. | `null` |
| `config.primary_dataset_name` | Name of the resource class corresponding to the embedded_dataset slot. | `null` |
...
| `config.publishable_artifacts` | List of artifacts to be published in their entirety when loaded into the Loader API. | `null` |
| `config.log_level` | The minimum log level to capture. | `"INFO"` |
| `config.service_name` | NOTE: this chart's configmap.tpl always overwrites config.service_name with the value computed from `serviceName` - a value set directly under config.service_name is silently discarded. Set `serviceName` instead. | `"metldata"` |
...
| `config.artifact_topic` | Name of the event topic containing artifact events | `null` |
...
| `configPrefix` | Prefix for the generated CONFIG_YAML env var and every Vault Agent-injected env var; create_charts.py derives this automatically from the package name | `"metldata"` |
...
> Docker Hub caps this overview at 25000 characters, so the table above stops partway through this chart's parameters. The full list, with defaults and descriptions, is in [values.schema.json](https://github.com/ghga-de/ghga/blob/main/deploy/charts/metldata/values.schema.json).
