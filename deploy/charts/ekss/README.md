...
Encryption Key Store Service - providing crypt4gh file secret extraction, storage and envelope generation

## Installing

```
helm install ekss oci://registry-1.docker.io/ghga/ekss-chart
```

## Source

Part of the [GHGA monorepo](https://github.com/ghga-de/ghga/tree/main/services/ekss). See
...
| `executable` | Executable name and arguments (will be combined into a shell command) | `"ekss"` |
| `executableArgs` |  | `[]` |
...
| `image.repository` | Image repository path (create_charts.py fills this in per member) | `"ghga/ekss"` |
...
| `_topics` | Per-instance Kafka topics this chart owns (merged with global._topics); shape is {<key>: {topic: {name, value}, type: {name, value}, types: [...]}} - see kafkaTopicsParameters/kafkaUser below for how each field is consumed | `{}` |
...
| `serviceName` | Logical service name; combined with serviceNamePrefix and injected into config.yaml as service_name | `"ekss-rest"` |
...
| `config.server_private_key_path` | Path to the Crypt4GH private key file | `null` |
| `config.vault_url` | URL of the vault instance to connect to | `null` |
| `config.vault_path` | Path without leading or trailing slashes where secrets should be stored in the vault. | `null` |
...
| `config.private_key_passphrase` | Passphrase needed to read the content of the private key file. Only needed if the private key is encrypted. | `null` |
| `config.log_level` | The minimum log level to capture. | `"INFO"` |
| `config.service_name` | NOTE: this chart's configmap.tpl always overwrites config.service_name with the value computed from `serviceName` - a value set directly under config.service_name is silently discarded. Set `serviceName` instead. | `"ekss"` |
...
| `config.vault_role_id` | Vault role ID to access a specific prefix | `null` |
| `config.vault_secret_id` | Vault secret ID to access a specific prefix | `null` |
| `config.vault_verify` | SSL certificates (CA bundle) used to verify the identity of the vault, or True to use the default CAs, or False for no verification. | `true` |
| `config.vault_secrets_mount_point` | Name used to address the secret engine under a custom mount path. | `"secret"` |
...
| `configPrefix` | Prefix for the generated CONFIG_YAML env var and every Vault Agent-injected env var; create_charts.py derives this automatically from the package name | `"ekss"` |
