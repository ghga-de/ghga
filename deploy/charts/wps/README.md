...
| `executable` | Executable name and arguments (will be combined into a shell command) | `"wps"` |
...
| `image.repository` | Image repository path (create_charts.py fills this in per member) | `"ghga/wps"` |
...
| `migrationInitContainer.executable` | Executable name and arguments run inside the migration init container | `"wps"` |
...
| `_topics.researchDataUploadBoxTopic.kafkaUser.operations` |  | `["Read"]` |
| `_topics.datasetChangeEvent.topic.name` |  | `"dataset_change_topic"` |
| `_topics.datasetChangeEvent.topic.value` |  | `"metadata-datasets"` |
| `_topics.datasetChangeEvent.types` |  | `[{"name": "dataset_upsertion_type", "value": "dataset_created"}, {"name": "dataset_deletion_type", "value": "dataset_deleted"}]` |
...
| `_topics.accessionMapTopic.kafkaUser.operations` |  | `["Read"]` |
...
| `mongodb.dbName` | Database name; combined with dbNamePrefix and injected into config.yaml as db_name. NOTE: mongodb.dbName is the fallback used when the top-level dbName (set per-member, not defaulted here) is empty | `"work-package"` |
...
| `apiBasePath` | Public API path prefix; combined with apiBasePathPrefix (set by an aliasing umbrella) and injected into config.yaml as api_root_path | `"/wps/"` |
| `serviceName` | Logical service name; combined with serviceNamePrefix and injected into config.yaml as service_name | `"wps-consumer"` |
...
| `config.db_name` | NOTE: this chart's configmap.tpl always overwrites config.db_name with the value computed from `mongodb.dbName` - a value set directly under config.db_name is silently discarded. Set `mongodb.dbName` instead. | `"work-packages"` |
| `config.access_url` | Base URL of the internal access API for download and upload | `null` |
...
| `config.service_name` | NOTE: this chart's configmap.tpl always overwrites config.service_name with the value computed from `serviceName` - a value set directly under config.service_name is silently discarded. Set `serviceName` instead. | `"wps"` |
| `config.work_package_signing_key` | The private key for signing work order tokens | `null` |
...
| `config.datasets_collection` | The name of the database collection for datasets | `"datasets"` |
| `config.upload_boxes_collection` | The name of the database collection for upload boxes | `"uploadBoxes"` |
| `config.work_packages_collection` | The name of the database collection for work packages | `"workPackages"` |
| `config.accession_maps_collection` | The name of the database collection for file accession maps | `"accessionMaps"` |
| `config.work_package_valid_days` | How many days a work package (and its access token) stays valid | `30` |
