...
```
helm install dins oci://registry-1.docker.io/ghga/dins-chart
```
...
| `executable` | Executable name and arguments (will be combined into a shell command) | `"dins"` |
...
| `_topics.fileRegisteredEvent.kafkaUser.operations` |  | `["Read"]` |
| `_topics.filesToDelete.topic.name` |  | `"file_deletion_request_topic"` |
| `_topics.filesToDelete.topic.value` |  | `"purges"` |
| `_topics.filesToDelete.types` |  | `[{"name": "file_deletion_request_type", "value": "file_deletion_requested"}]` |
| `_topics.filesToDelete.kafkaUser.operations` |  | `["Read"]` |
| `_topics.datasetUpsertionEvent.topic.name` |  | `"dataset_change_topic"` |
| `_topics.datasetUpsertionEvent.topic.value` |  | `"metadata-datasets"` |
| `_topics.datasetUpsertionEvent.types` |  | `[{"name": "dataset_upsertion_type", "value": "dataset_created"}]` |
| `_topics.datasetUpsertionEvent.kafkaUser.operations` |  | `["Read"]` |
| `_topics.datasetDeletionEvent.topic.name` |  | `"dataset_change_topic"` |
| `_topics.datasetDeletionEvent.topic.value` |  | `"metadata-datasets"` |
| `_topics.datasetDeletionEvent.types` |  | `[{"name": "dataset_deletion_type", "value": "dataset_deleted"}]` |
| `_topics.datasetDeletionEvent.kafkaUser.operations` |  | `["Write"]` |
...
| `mongodb.dbName` | Database name; combined with dbNamePrefix and injected into config.yaml as db_name. NOTE: mongodb.dbName is the fallback used when the top-level dbName (set per-member, not defaulted here) is empty | `"dataset-information"` |
...
| `apiBasePath` | Public API path prefix; combined with apiBasePathPrefix (set by an aliasing umbrella) and injected into config.yaml as api_root_path | `"/dins/"` |
| `serviceName` | Logical service name; combined with serviceNamePrefix and injected into config.yaml as service_name | `"dins-consumer"` |
...
| `config.service_name` | NOTE: this chart's configmap.tpl always overwrites config.service_name with the value computed from `serviceName` - a value set directly under config.service_name is silently discarded. Set `serviceName` instead. | `"dins"` |
